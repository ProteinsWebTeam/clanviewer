...
describe('#processData(data)', () => {
    ...
    it('should use num_full only if score is not included', () => {
        const dataJson = {
            members:[
                {accession:"ipr1", name:"id1", score: 0, num_full:2},
                {accession:"ipr2", name:"id2", num_full:2},
            ],
            relationships:[
                { source:"ipr1", target:"ipr2", score: 8.2e-6 }
            ]
        };
        const obj = new ClanViewer({r: 5});

        obj.processData(dataJson);
        expect(dataJson.nodes[0].score).toBe(0);
        expect(dataJson.nodes[1].score).toBe(2);
    })
});
...
describe('paint()', () => {
    ...
    it('should paint multiple relationships', function(){
        ...
        expect(dataJson.relationships).toHaveLength(d3.select(root).selectAll(".link").size());
        expect(dataJson.members).toHaveLength(d3.select(root).selectAll(".node").size());
