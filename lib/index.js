...
export default class ClanViewer {
    constructor({
        ...
        this.zoom = d3Zoom()
            .filter(() => {
                ...
                return !this.useCtrlToZoom || d3Event.ctrlKey;
            })
            .scaleExtent([0.5, 10])
            .on("zoom", () => {
    ...
    _autoUpdateSize() {
        const parent = select(this.element);
    ...
    paint(data, is_data_processed=false) {
        ...
        const linkGData = this.linksgroup.selectAll(".link")
            .data(data.links);

        const linkG = linkGData.enter().append("g")
    ...
    processData(data) {
        ...
        data.nodes = data.members.map(e => ({
            ...
            score: Number.parseFloat(('score' in e &&  e.score !== null)? e.score : e.num_full),
        }));
        ...
        if (this.multiple_relationships) {
            data.relationships.forEach(e => {
                ...
                e.score = Number.parseFloat(('score' in e &&  e.score !== null) ? e.score : e.evalue);
            });
